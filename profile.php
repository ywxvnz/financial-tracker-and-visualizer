  ...
  <div class="profile-container">
    ...
    <div class="profile-area">
      ...
      <div class="profile-infos">
        ...
        <p id="profile-upload-msg" style="color: green; font-weight: bold;"></p>
        <div>
    ...
    <div class="forms-section">
          ...
          <input type="number" step="0.01" id="current_balance" name="current_balance"
                 value="<?php echo htmlspecialchars($financials['balance'] ?? ''); ?>" placeholder="₱0.00" />

          <label>Current Savings</label>
          <input type="number" step="0.01" id="savings" name="savings"
                 value="<?php echo htmlspecialchars($financials['savings'] ?? ''); ?>" placeholder="₱0.00" />

          <label>Estimated Monthly Income</label>
          <input type="number" step="0.01" id="income_amount" name="income_amount"
                 value="<?php echo htmlspecialchars($financials['income'] ?? ''); ?>" placeholder="Monthly - ₱0.00" />

          <label>Estimated Monthly Expenses</label>
          <input type="number" step="0.01" id="expense_amount" name="expense_amount"
                 value="<?php echo htmlspecialchars($financials['expenses'] ?? ''); ?>" placeholder="Monthly - ₱0.00" />

          <label>Loan Balance</label>
          <input type="number" step="0.01" id="loan_balance" name="loan_balance"
                 value="<?php echo htmlspecialchars($financials['loan'] ?? ''); ?>" placeholder="₱0.00" />

          <p id="form-error-msg" style="color: red; font-weight: bold;"></p>
          <?php
          if (isset($_SESSION['success_message'])) {
              echo '<p id="success-msg" style="color: green; font-weight: bold;">' . htmlspecialchars($_SESSION['success_message']) . '</p>';
      ...
      profilePictureUpload.addEventListener('change', function() {
        ...
        if (file) {
          ...
          reader.onload = function(e) {
            ...
            uploadMsg.textContent = "Profile picture added successfully!";
            setTimeout(() => {
